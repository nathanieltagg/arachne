#define CutPlotSlice_cxx
#include "CutPlotSlice.h"
#include <TH2.h>
#include <TStyle.h>
#include<limits>
...
//      Root > .L CutPlotSlice.C
//
// and create an instance of the CutPlotSlice class with a DST file:
//      Root > CutPlotSlice t( filename.root )
//
...
void CutPlotSlice::InitHitCounter(){
  N_hit_slice[0] = new TH2F("N_hit_slice_X","N_hit_slice_X",100,-0.5,99.5,21,-1.5,19.5);
  N_hit_slice[1] = new TH2F("N_hit_slice_U","N_hit_slice_U",100,-0.5,99.5,21,-1.5,19.5);
  N_hit_slice[2] = new TH2F("N_hit_slice_V","N_hit_slice_V",100,-0.5,99.5,21,-1.5,19.5);
  // cout << " pointer " << N_hit_slice << endl;
}

void CutPlotSlice::FillHitCounter(double pecut){
  for (int ih = 0 ; ih < n_rawhits; ih++){
    if (hit_pe_cal[ih] < pecut) continue;
    float im = hit_module[ih];
    float is = hit_time_slice[ih];
    float istrip = hit_strip[ih];
    int iv = hit_view[ih]-1;
    if (iv < 0) continue;
    // only count central strips for later counters
    if(im > 76 && (istrip <= 10 || istrip > 118))continue; 
    //if(im < 74 || im > 80) continue;   
    //int ibin = N_hit_slice->FindBin(im,is);
    N_hit_slice[iv]->Fill(im,is);
    N_hit_slice[iv]->Fill(im,-1);
    
  }
}


int CutPlotSlice::GetHitCounter(int slice,int module, int view){
  int ibin = N_hit_slice[view]->FindBin(float(module),float(slice));
  return N_hit_slice[view]->GetBinContent(ibin);
}

void CutPlotSlice::ResetHitCounter(){
  N_hit_slice[0]->Reset();
  N_hit_slice[1]->Reset();
  N_hit_slice[2]->Reset();
}

void CutPlotSlice::LoopEventDisplay(int first_event, int num_events, int num_modules, int first_module, bool show_clusters, bool print_files, bool pause_between, double Cx, double Cy ){
  ...
  for (Long64_t jentry=first_event-1; jentry<nentries && jentry<first_event+num_events-1;jentry++) {
    ...
    // count the hits above 1 pe in first 3 modules for this slice
    ...
    int vhits = 0;
    int thits[3] = {0,0,0};
    for( int im = first_module; im < first_module+num_modules; im++){
      // cout << "Hits in module, slice " << im << " " << slice << " " <<  GetHitCounter(slice,im) << endl;
      for(int iv = 0; iv < 3; iv++){
	if(im < first_module+3)vhits += GetHitCounter(slice,im,iv);
	else thits[iv] += GetHitCounter(slice,im,iv);
      }
    }
    ResetHitCounter();
    ...
    for (int it = 0; it < n_tracks; it++){
      if(trk_nodes[it]>0){
	cout << "trk - x y x ax ay " << it << "\t" <<  trk_node_X[it][0] << "\t" << trk_node_Y[it][0] << "\t" <<  trk_node_Z[it][0] << "\t" << trk_node_aX[it][0] << "\t" << trk_node_aY[it][0] << "\t" << trk_node_chi2[it][0] << trk_nodes[it] << endl;  
      }
      //if(trk_node_Z[it] < -2000.) continue;
      if(trk_nodes[it] < 20)continue;
      ntk++;
    }
    cout << " thits, vhits " << thits[0] << "," << thits[1] << "," << thits[2] << "," << vhits << endl;

    bool showme = (thits[0] > 10)&& thits[1] > 5 && thits[2] > 5 && (vhits < 2);
    if(showme) event_display(gatenum, slice, pe_cut, 35, show_clusters, print_files, num_modules, first_module, cEventDisplay );
...
void CutPlotSlice::PauseEventDisplay(double pe_cut, int num_modules, int first_module, bool show_clusters, 
...
void CutPlotSlice::LoopClusterPEs(char* label, int first_event, int num_events, int num_modules, bool print_files){
...
void CutPlotSlice::LoopTimeCalib(int first_event, int num_events, int croc, int chain1, int chain2, double tmax, bool print_files){
...
void CutPlotSlice::event_display( const int gate, int slice, double pe_cut, double maxPE, 
...
void CutPlotSlice::raw_data( int event ){
...
void CutPlotSlice::add_plot_label( char* label, double x, double y, double size, int color  ){
...
void CutPlotSlice::set_root_env(){
...
void CutPlotSlice::set_color_pallete(){
...
int CutPlotSlice::pause( int &slice, double &pe_cut ){
