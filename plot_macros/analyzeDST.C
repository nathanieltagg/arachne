...
void analyzeDST::LoopEventDisplay(int first_event, int num_events, int num_modules, int first_module, bool show_clusters, 
				  bool print_files, bool pause_between, double Cx, double Cy ){
...
void analyzeDST::PauseEventDisplay(double pe_cut, int num_modules, int first_module, bool show_clusters, 
  ...
  for (Long64_t jentry=0; jentry<nentries; jentry++) {
    ...
    next = pause( new_slice, new_pe_cut );
 
    if( next == -2 || next == -3 ){  

      sprintf( filetype, "gif" );
      if( next == -3 ) sprintf( filetype, "jpg" );
   
      if( slice < 0 || slice > n_slices ){
	if( pe_cut == 0 )
	  sprintf(words,"event_display_run%d_sub%d_ev%d.%s",ev_run, ev_sub_run, gatenum, filetype );
	else
	  sprintf(words,"event_display_run%d_sub%d_ev%d_c%2.1f.%s",ev_run, ev_sub_run, gatenum, pe_cut, filetype );
      }else{
	if( pe_cut == 0 )
	  sprintf(words,"event_display_run%d_sub%d_ev%d_sl%d.%s",ev_run, ev_sub_run, gatenum, slice, filetype );
	else
	  sprintf(words,"event_display_run%d_sub%d_ev%d_sl%d_c%2.1f.%s",ev_run, ev_sub_run, gatenum, slice, pe_cut, filetype );
      }

      cEventDisplay->Print(words, filetype);
...
void analyzeDST::LoopTimeCalib(int first_event, int num_events, int croc, int chain1, int chain2, double tmax, bool print_files){
  ...
  if( print_files == 1 ){
    sprintf( filename, "run%d_subrun%d_croc%d_chains%d-%d_ticks_allhits.gif",ev_run,ev_sub_run,croc,chain1,chain2);
    cT1->Print(filename,"gif");
  }
  ...
  if( print_files == 1 ){
    sprintf( filename, "run%d_subrun%d_croc%d_chains%d-%d_time_allhits.gif",ev_run,ev_sub_run,croc,chain1,chain2);
    cT2->Print(filename,"gif");
  }
  ...
  if( print_files == 1 ){
    sprintf( filename, "run%d_subrun%d_croc%d_chains%d-%d_timing.gif",ev_run,ev_sub_run,croc,chain1,chain2);
    cT3->Print(filename,"gif");
  }
  ...
  if( print_files == 1 ){
    sprintf( filename, "run%d_subrun%d_croc%d_chains%d-%d_lego.gif",ev_run,ev_sub_run,croc,chain1,chain2);
    cTiming_Lego->Print(filename,"gif");
  }
  ...
  if( print_files == 1 ){
    sprintf( filename, "run%d_subrun%d_croc%d_chains%d-%d_tres_chains.gif",ev_run,ev_sub_run,croc,chain1,chain2);
    cT5->Print(filename,"gif");
  }
  ...
  if( print_files == 1 ){
    sprintf( filename, "run%d_subrun%d_croc%d_chains%d-%d_charge.gif",ev_run,ev_sub_run,croc,chain1,chain2);
    cT4->Print(filename,"gif");
  }
...
void analyzeDST::event_display( const int gate, int slice, double pe_cut, double maxPE, 
  ...
  if( print_files == 1 ){
    sprintf(words,"event_display_run%d_sub%d_ev%d.gif",ev_run, ev_sub_run, gate);
    cEventDisplay->Print(words,"gif");
  }
  else
...
int analyzeDST::pause( int &slice, double &pe_cut ){
  ...
  cout << "  add a '-X' to any command to make a min pe cut on display" << endl;
  cout << "     -Ex.  131-1.5  or  250/3-2.5" << endl; 
  cout << "  'p' to print the current display to .gif" << endl;
  cout << "  'j' to print the current display to .jpg" << endl;
  ...
  //-- fill spill number before / or -
  for( i = 0; i < del1 && i < del2; i++ ){
    evstr[i] = junk[i];
  }

  //-- slice is between / and -
