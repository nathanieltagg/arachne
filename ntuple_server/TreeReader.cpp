...
JsonArray   TreeReader::getJsonArray(const std::string& leafname)
{
  JsonArray arr;
  
  TLeaf* leaf = fTree->GetLeaf(leafname.c_str());
  if(!leaf) return arr; // null element.

  Int_t n = leaf->GetNdata();
  
  for(int i=0;i<n;++i) {
    arr.add(getJson(leaf,i));
  }
  return arr;
  
}

JsonArray   TreeReader::getJsonArray(const std::string& leafname, int index)
{
  JsonArray arr;
  TLeaf* leaf = fTree->GetLeaf(leafname.c_str());
  if(!leaf) return arr; // null element.
