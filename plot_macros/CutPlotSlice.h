#ifndef CutPlotSlice_h
#define CutPlotSlice_h
...
void FillHitCounter( double pecut=0.1);

int GetHitCounter( int slice,int module, int iv);

void ClearHitCounter();

 TH2F* N_hit_slice[3]; 

   CutPlotSlice(char* filename);
   virtual ~CutPlotSlice();
   ...
   //-- EventDisplay
   virtual void     LoopEventDisplay(int first_event = 1, int num_events = 10000, int num_modules = 24, int first_module = 74,
   ...
   virtual void     PauseEventDisplay(double pe_cut = 0, int num_modules = 24, int first_module = 74, 
				      bool show_clusters = false, bool print_files = true, double Cx = 1200, double Cy = 800 );
...
#ifdef CutPlotSlice_cxx


//== constructor
CutPlotSlice::CutPlotSlice( char* filename ){
...
Int_t CutPlotSlice::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t CutPlotSlice::LoadTree(Long64_t entry)
{
...
void CutPlotSlice::Init(TTree *tree)
{
...
void CutPlotSlice::Show(Long64_t entry)
{
...
Int_t CutPlotSlice::Cut(Long64_t entry)
{
